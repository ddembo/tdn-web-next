...
const ButtonLink = ({ children, href, variant, inline, nav }: ButtonLinkProps) => {
  ...
  const variantStyles: { [k in ButtonLinkVariant]?: string } = {
    ...
    subtle:
      'underline bg-transparent shadow-none before:bg-tdn-highlight hover:bg-tdn-secondary hover:shadow-none hover:no-underline',
  };
  const inlineStyles = 'inline-block px-2 py-2 md:px-2 md:py-2 md:pt-1 pt-1';
